...
class ProposalExporter:
    ...
    def _render_markdown(self, draft: ProposalDraft) -> str:
        ...
        overall_status = "complete" if draft.completeness.complete else "incomplete"
        ...
        for check in draft.completeness.checks:
            ...
            if check.missing_fields:
                lines.append(
                    f"- {check.title}: {status} "
                    f"(missing: {', '.join(check.missing_fields)})"
                )
            else:
