...
def test_table_pipeline_extracts_captioned_table_from_pdf_text(tmp_path: Path) -> None:
    from paperbase.db.models import TableArtifact
    from paperbase.tables.pipeline import TableExtractionPipeline
    ...
    result = TableExtractionPipeline(session_factory=session_factory).extract_and_store(paper_id)
    ...
    assert tables[0].table_label == "Table 1"
    assert tables[0].caption == "Benchmark comparison across scRegNet methods."
    assert tables[0].storage_uri == f"{pdf_path.resolve().as_uri()}#page=1"
    assert tables[0].structured_payload_json == {}
