...
def test_job_list_returns_recent_jobs_for_ui_polling(tmp_path) -> None:
    database_path = tmp_path / "paperbase.sqlite3"
    initialize_database(f"sqlite:///{database_path}")
    session_factory = make_session_factory(f"sqlite:///{database_path}")
    client = TestClient(create_app(session_factory=session_factory))

    first_job = client.post("/api/v1/search/reindex")
    second_job = client.post("/api/v1/search/reindex")

    assert first_job.status_code == 202
    assert second_job.status_code == 202

    response = client.get("/api/v1/jobs", params={"limit": 10})

    assert response.status_code == 200
    payload = response.json()["data"]
    assert len(payload) == 2
    assert payload[0]["id"] == second_job.json()["data"]["id"]
    assert payload[1]["id"] == first_job.json()["data"]["id"]
    assert payload[0]["job_type"] == "search_reindex"
