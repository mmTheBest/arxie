...
def test_run_command_starts_stack_waits_for_ready_and_opens_browser(monkeypatch) -> None:
    ...
    assert compose_calls == [
        ...
        ["up", "-d", "--force-recreate", "paperbase-api", "paperbase-worker"],
    ]
...
def test_run_command_can_rebuild_runtime_images_when_requested(monkeypatch) -> None:
    runner = CliRunner()
    compose_calls: list[list[str]] = []
    ...
    result = runner.invoke(app, ["run", "--rebuild"])

    assert result.exit_code == 0
    assert compose_calls == [
        ["up", "-d", "postgres", "minio", "redis"],
        ["build", "paperbase-migrate", "paperbase-api", "paperbase-worker"],
        ["run", "--rm", "paperbase-migrate"],
        ["up", "-d", "--force-recreate", "paperbase-api", "paperbase-worker"],
    ]
