import { fetchJson, withProjectHeaders } from "./api-client.js";
import { artifactDownloadText } from "./artifact-export.js";
...
import { elements, endpoints, state } from "./state.js";
import {
  escapeHtml,
  formatValue,
  isActiveJobStatus,
  ...
  function setCurrentProject(project) {
    ...
    if (state.currentProject && state.currentProject.id) {
      sessionStorage.setItem("arxie.currentProjectId", state.currentProject.id);
    } else {
      sessionStorage.removeItem("arxie.currentProjectId");
    }
    renderProjects();
  }

  function renderProjects() {
    if (!elements.currentProjectSummary || !elements.recentProjectsList) {
      return;
    }
    if (state.currentProject) {
      elements.currentProjectSummary.textContent = `${state.currentProject.title} · ${state.currentProject.root_path}`;
    } else {
      elements.currentProjectSummary.textContent = "Default local library";
    }

    if (state.projects.length === 0) {
      elements.recentProjectsList.innerHTML = '<div class="list-card muted">No recent projects yet.</div>';
      return;
    }
    elements.recentProjectsList.innerHTML = state.projects.map((project) => {
      const active = state.currentProject && state.currentProject.id === project.id;
      return `
        <div class="list-card list-card-compact" data-active="${active ? "true" : "false"}">
          <button type="button" data-project-open-id="${escapeHtml(project.id)}">
            <div class="list-title">${escapeHtml(project.title)}</div>
            <div class="muted">${escapeHtml(project.root_path)}</div>
          </button>
        </div>
      `;
    }).join("");
    elements.recentProjectsList.querySelectorAll("[data-project-open-id]").forEach((button) => {
      button.addEventListener("click", async () => {
        const projectId = button.getAttribute("data-project-open-id");
  ...
  function visiblePaperMemberships() {
    const searchResultIds = new Set(state.searchResults.map((paper) => paper.id));
    const sourceFilter = elements.paperSourceSearchInput
      ? elements.paperSourceSearchInput.value.trim().toLowerCase()
      : "";
    return state.papers.filter((membership) => {
      const paper = membership.paper;
      if (searchResultIds.size > 0 && !searchResultIds.has(paper.id)) {
        return false;
      }
      if (!sourceFilter) {
        return true;
      ...
      return searchable.includes(sourceFilter);
    });
  }

  function renderPaperSources() {
    if (!elements.paperSourceList) {
      return;
    }
    const items = visiblePaperMemberships();
    if (!state.selectedCollection) {
      elements.paperSourceList.innerHTML = '<div class="list-card muted">Select a collection to begin.</div>';
      return;
    }
    if (items.length === 0) {
      elements.paperSourceList.innerHTML = '<div class="list-card muted">No papers match the current source filter.</div>';
      return;
    }

    elements.paperSourceList.innerHTML = items
      .map((membership) => {
        const paper = membership.paper;
        ...
        return `
          <div class="paper-source-card" data-active="${active ? "true" : "false"}" data-status="${escapeHtml(processing.status)}">
            <label class="paper-select-control">
              <input
                type="checkbox"
                data-paper-source-checkbox="${paper.id}"
                ${checked ? "checked" : ""}
              />
              <span class="status-marker" data-status="${escapeHtml(processing.status)}"></span>
              <span class="paper-row-copy">
                <span class="list-title">${escapeHtml(paper.title)}</span>
                <span class="muted">${escapeHtml(meta || paper.provider || "Paper record")}</span>
                <span class="muted">${escapeHtml(authorLine || "Unknown authors")} · ${escapeHtml(processing.label)}</span>
              </span>
            </label>
            <button type="button" class="source-open-button" data-paper-source-id="${paper.id}">
              Open
            </button>
          </div>
        `;
    ...
    elements.paperSourceList.querySelectorAll("[data-paper-source-checkbox]").forEach((checkbox) => {
      checkbox.addEventListener("change", () => {
        const paperId = checkbox.getAttribute("data-paper-source-checkbox");
        if (!paperId) {
          return;
        }
        if (checkbox.checked) {
    ...
    elements.paperSourceList.querySelectorAll("[data-paper-source-id]").forEach((button) => {
      button.addEventListener("click", async () => {
  ...
  function renderPaperDetail() {
    ...
    elements.paperDetail.innerHTML = `
      ...
      <div class="button-row button-row-left">
        <button type="button" class="action-button action-button-primary" data-paper-research-action="focus">
          Focus chat on this paper
        </button>
  ...
  function renderSearchSurfaces() {
    if (!elements.chunkSearchSurface || !elements.artifactSearchSurface) {
      return;
    }
  ...
  function renderResearchSuggestions() {
    if (!elements.researchSuggestionList) {
      return;
    }
    if (!state.selectedCollection) {
      elements.researchSuggestionList.innerHTML = "";
      return;
    }
    if (!state.research.suggestions || state.research.suggestions.length === 0) {
      elements.researchSuggestionList.innerHTML = '<div class="muted">Suggestions will appear after a collection is selected.</div>';
      return;
    }
    elements.researchSuggestionList.innerHTML = state.research.suggestions.map((suggestion) => `
      <button
        type="button"
        class="suggestion-chip"
        data-research-suggestion-id="${escapeHtml(suggestion.id)}"
        title="${escapeHtml(suggestion.instruction)}"
      >
        ${escapeHtml(suggestion.label)}
      </button>
    `).join("");
    elements.researchSuggestionList.querySelectorAll("[data-research-suggestion-id]").forEach((button) => {
      button.addEventListener("click", async () => {
        const suggestionId = button.getAttribute("data-research-suggestion-id");
        const suggestion = state.research.suggestions.find((item) => item.id === suggestionId);
        if (!suggestion) {
          return;
        }
        state.research.selectedSuggestionId = suggestion.id;
        elements.researchMessageInput.value = suggestion.instruction;
        elements.researchMessageInput.focus();
      });
    });
  }

  function formatArtifactType(value) {
    return String(value || "artifact").replace(/_/g, " ");
  }

  function artifactsForSelectedThread() {
    if (!state.research.selectedThread) {
      return state.research.artifacts;
    }
    return state.research.artifacts.filter(
      (artifact) => artifact.thread_id === state.research.selectedThread.id,
    );
  }

  function artifactSaveFormats(artifact) {
    const output = artifact.output_payload || {};
    const declaredOptions = Array.isArray(output.save_options) ? output.save_options : [];
    const fallback = artifact.artifact_type === "comparison"
      ? ["markdown", "csv"]
      : ["markdown", "note"];
    return uniqueStrings(declaredOptions.length > 0 ? declaredOptions : fallback)
      .filter((format) => ["markdown", "csv", "script", "note"].includes(format));
  }

  function renderResearchArtifactActions(artifact) {
    const formats = artifactSaveFormats(artifact);
    const disabled = artifact.status !== "completed";
    return `
      <div class="artifact-action-row">
        ${formats.map((format) => `
          <button
            type="button"
            class="action-button"
            data-artifact-save-id="${escapeHtml(artifact.id)}"
            data-artifact-save-format="${escapeHtml(format)}"
            ${disabled ? "disabled" : ""}
          >
            Save ${escapeHtml(format)}
          </button>
        `).join("")}
        <button
          type="button"
          class="action-button"
          data-artifact-download-id="${escapeHtml(artifact.id)}"
          data-artifact-download-format="${escapeHtml(formats[0] || "markdown")}"
          ${disabled ? "disabled" : ""}
        >
          Export
        </button>
  ...
  function renderResearchArtifactCard(artifact) {
    return `
      <div class="research-artifact-card" data-status="${escapeHtml(artifact.status)}">
        <button type="button" class="artifact-card-main" data-research-artifact-id="${escapeHtml(artifact.id)}">
          <span class="job-status" data-status="${escapeHtml(artifact.status)}">${escapeHtml(artifact.status)}</span>
          <span class="list-title">${escapeHtml(artifact.saved_title || artifact.title)}</span>
          <span class="muted">${escapeHtml(formatArtifactType(artifact.artifact_type))}</span>
          ${renderResearchArtifactPreview(artifact)}
          ${renderResearchRunStatus(artifact)}
        </button>
        ${renderResearchArtifactActions(artifact)}
      </div>
    `;
  }

  function bindResearchArtifactActions(root) {
    if (!root) {
      return;
    }
    root.querySelectorAll("[data-research-artifact-id]").forEach((button) => {
      button.addEventListener("click", async () => {
        ...
        try {
          await loadResearchArtifactRun(artifactId, { render: false });
          renderResearchWorkspace();
        } catch (error) {
          setStatus(error.message);
        }
    ...
    root.querySelectorAll("[data-artifact-save-id]").forEach((button) => {
      button.addEventListener("click", async () => {
        const artifactId = button.getAttribute("data-artifact-save-id");
        const format = button.getAttribute("data-artifact-save-format") || "markdown";
        if (!artifactId) {
          return;
        }
        try {
          await saveResearchArtifact(artifactId, format);
        } catch (error) {
          setStatus(error.message);
        }
      });
    });
    root.querySelectorAll("[data-artifact-download-id]").forEach((button) => {
      button.addEventListener("click", () => {
        const artifactId = button.getAttribute("data-artifact-download-id");
        const format = button.getAttribute("data-artifact-download-format") || "markdown";
        if (!artifactId) {
          return;
        }
        try {
          downloadResearchArtifact(artifactId, format);
        } catch (error) {
          setStatus(error.message);
        }
      });
  ...
  function renderSavedOutputs() {
    if (!elements.savedOutputList) {
      return;
    }
    const savedArtifacts = state.research.artifacts.filter((artifact) => artifact.is_saved);
    if (savedArtifacts.length === 0) {
      elements.savedOutputList.innerHTML = '<div class="list-card muted">Save a chat result to keep it here.</div>';
      return;
    }
    elements.savedOutputList.innerHTML = savedArtifacts.map((artifact) => `
      <div class="list-card" data-active="${state.research.selectedArtifact && state.research.selectedArtifact.id === artifact.id ? "true" : "false"}">
        <button type="button" data-research-artifact-id="${escapeHtml(artifact.id)}">
          <div class="job-status" data-status="${escapeHtml(artifact.status)}">${escapeHtml(artifact.saved_format || "saved")}</div>
          <div class="list-title">${escapeHtml(artifact.saved_title || artifact.title)}</div>
          <div class="muted">${escapeHtml(formatArtifactType(artifact.artifact_type))}</div>
        </button>
        <div class="artifact-action-row">
          <button
            type="button"
            class="action-button"
            data-artifact-download-id="${escapeHtml(artifact.id)}"
            data-artifact-download-format="${escapeHtml(artifact.saved_format || "markdown")}"
          >
            Export
          </button>
        </div>
    ...
    `).join("");
    bindResearchArtifactActions(elements.savedOutputList);
  }
  ...
  function renderResearchWorkspace() {
    ...
    if (!state.selectedCollection) {
      ...
      if (elements.researchArtifactList) {
        elements.researchArtifactList.innerHTML = '<div class="list-card muted">No chat outputs yet.</div>';
      }
      renderSavedOutputs();
    ...
    const threadArtifacts = artifactsForSelectedThread();
    const messageHtml = state.research.messages.length > 0
      ...
      `).join("")
      : '<div class="muted">Ask for an experiment plan, hypotheses, critique, comparison, or field patterns.</div>';
    const artifactHtml = threadArtifacts.length > 0
      ? threadArtifacts.map(renderResearchArtifactCard).join("")
      : "";
    elements.researchChatLog.innerHTML = messageHtml + artifactHtml;
    bindResearchArtifactActions(elements.researchChatLog);

    if (elements.researchArtifactList) {
      const unsavedArtifacts = threadArtifacts.filter((artifact) => !artifact.is_saved);
      elements.researchArtifactList.innerHTML = unsavedArtifacts.length > 0
        ? '<div class="list-card muted">Unsaved chat results stay in the conversation until you save them.</div>'
        : '<div class="list-card muted">Saved outputs will appear above.</div>';
    }
  ...
  function renderResearchArtifactPreview(artifact) {
    ...
    if (artifact.artifact_type === "comparison") {
      const rows = output.comparison_rows || [];
      const rowSummary = rows.slice(0, 2).map((item) => {
        return [item.method, item.dataset, item.metric].filter(Boolean).join(" / ");
      }).filter(Boolean).join(" · ");
  ...
  function storeResearchRun(run) {
    if (!run || !run.id) {
      return null;
    }
    state.research.runs[run.id] = run;
    if (run.artifact_id) {
      state.research.runsByArtifact[run.artifact_id] = run.id;
      const artifact = state.research.artifacts.find((item) => item.id === run.artifact_id);
      if (artifact) {
        artifact.input_payload = {
          ...(artifact.input_payload || {}),
          run_id: run.id,
        };
      }
      if (state.research.selectedArtifact && state.research.selectedArtifact.id === run.artifact_id) {
        state.research.selectedArtifact.input_payload = {
          ...(state.research.selectedArtifact.input_payload || {}),
          run_id: run.id,
        };
      }
  ...
  function runForArtifact(artifact) {
    if (!artifact) {
      return null;
    }
    const runId = (artifact.evidence_payload && artifact.evidence_payload.run_id)
      || (artifact.input_payload && artifact.input_payload.run_id)
      || state.research.runsByArtifact[artifact.id];
    if (!runId) {
      return null;
    }
    return state.research.runs[runId] || null;
  }

  function renderResearchRunStatus(artifact) {
    const run = runForArtifact(artifact);
    if (!run) {
      return '<div class="muted">Run trace will appear after the worker starts.</div>';
    }
    const validation_report = run.validation_report;
    const context = run.context_pack;
    const stepSummary = (run.steps || []).slice(-3).map((step) => `
      <li>
        <span>${escapeHtml(step.label || step.step_type)}</span>
        <span class="job-status" data-status="${escapeHtml(step.status)}">${escapeHtml(step.status)}</span>
      </li>
    `).join("");
    return `
      <div class="research-run-status">
        <div class="muted">
          Run ${escapeHtml(run.status)} · ${escapeHtml(run.skill_id)}
          ${context ? ` · ${escapeHtml(context.context_summary.paper_count || 0)} papers` : ""}
        </div>
        <div class="muted">
          Validation: ${escapeHtml(validation_report ? validation_report.harness_status : "waiting")}
          ${run.error_message ? ` · ${escapeHtml(run.error_message)}` : ""}
        </div>
        ${stepSummary ? `<ul class="detail-list compact-list">${stepSummary}</ul>` : ""}
      </div>
    `;
  }
  ...
  function renderResearchEvidenceDrawer() {
    ...
    const runStatus = renderResearchRunStatus(artifact);
  ...
  function renderSettings() {
    if (!elements.settingsSummary || !elements.settingsRuntimeSummary) {
      return;
    }
    ...
    elements.settingsSummary.innerHTML = `
      <div class="detail-group">
        ...
        <div class="muted">${escapeHtml((state.currentProject && state.currentProject.title) || "Default local library")}</div>
      </div>
      <div class="detail-group">
  ...
  function updateActionButtons() {
    ...
    if (elements.extractUnprocessedPapersButton) {
    ...
    if (elements.saveWorkspaceButton) {
      elements.saveWorkspaceButton.disabled = !collectionSelected;
    }
    if (elements.researchSendButton) {
  ...
  async function loadProjects() {
    ...
    const storedProjectId = sessionStorage.getItem("arxie.currentProjectId");
    if (storedProjectId) {
      const match = state.projects.find((project) => project.id === storedProjectId);
      if (match) {
  ...
  async function openProjectFromForm() {
    ...
    const body = {
      root_path: rootPath,
      title: elements.projectTitleInput.value.trim() || null,
    };
    const response = await fetchJson(`${endpoints.projects}/open`, {
      method: "POST",
  ...
  async function loadPaperDetail(paperId) {
    const [paperPayload, structuredPayload, tablesPayload] = await Promise.all([
      fetchJson(`/api/v1/papers/${paperId}`),
      fetchJson(`/api/v1/papers/${paperId}/structured-data`),
      fetchJson(`/api/v1/papers/${paperId}/tables`),
    ]);

    state.selectedPaper = paperPayload.data;
    state.selectedPaperStructured = {
      ...structuredPayload.data,
      tables: tablesPayload.data || structuredPayload.data.tables || [],
    };
    renderAll();
  }
  ...
  async function loadResearchContext(options) {
    ...
    const [threadsPayload, artifactsPayload, labelsPayload, suggestionsPayload] = await Promise.all([
      ...
      fetchJson(`${endpoints.researchSuggestions}?${params.toString()}`),
    ]);
    ...
    state.research.suggestions = suggestionsPayload.data || [];
    state.research.runs = state.research.runs || {};
    state.research.runsByArtifact = state.research.runsByArtifact || {};
    if (
    ...
    if (state.research.selectedArtifact) {
      try {
        await loadResearchArtifactRun(state.research.selectedArtifact.id, { render: false });
      } catch (error) {
        // Older artifacts may not have a first-class run trace yet.
      }
    }
    if (!options || options.render !== false) {
      renderResearchWorkspace();
  ...
  async function loadResearchRun(runId, options) {
    if (!runId) {
      throw new Error("Research run id is required.");
    }
    const payload = await fetchJson(`${endpoints.researchRuns}/${runId}`);
    const run = storeResearchRun(payload.data);
    if (!options || options.render !== false) {
      renderResearchWorkspace();
    }
    return run;
  ...
  async function loadResearchArtifactRun(artifactId, options) {
    if (!artifactId) {
      throw new Error("Research artifact id is required.");
    }
    const payload = await fetchJson(`${endpoints.researchArtifacts}/${artifactId}/run`);
    const run = storeResearchRun(payload.data);
    if (!options || options.render !== false) {
      renderResearchWorkspace();
    }
  ...
  async function openResearchThread(threadId, options) {
    ...
    const threadArtifacts = state.research.artifacts.filter((artifact) => artifact.thread_id === threadId);
    if (
      state.research.selectedArtifact
      && !threadArtifacts.some((artifact) => artifact.id === state.research.selectedArtifact.id)
    ) {
      state.research.selectedArtifact = null;
    }
    if (!state.research.selectedArtifact && threadArtifacts.length > 0) {
      state.research.selectedArtifact = threadArtifacts[0];
    }
    if (state.research.selectedArtifact) {
      try {
        await loadResearchArtifactRun(state.research.selectedArtifact.id, { render: false });
  ...
  async function ensureResearchThread() {
    ...
    const selectedPaperSignature = selectedPaperIds.slice().sort().join("|");
    if (
      ...
      && (state.research.selectedThread.selected_paper_ids || []).slice().sort().join("|") === selectedPaperSignature
    ) {
  ...
  async function postResearchMessage() {
    ...
    const response = await fetchJson(`${endpoints.researchThreads}/${thread.id}/messages`, {
      ...
      body: JSON.stringify({
        message,
        suggestion_id: state.research.selectedSuggestionId,
    ...
    state.research.selectedSuggestionId = null;
    state.jobs.unshift(response.data.job);
    state.research.messages.push(response.data.message);
    const artifact = {
      ...response.data.artifact,
      input_payload: {
        ...(response.data.artifact.input_payload || {}),
        run_id: response.data.run_id,
      },
    };
    state.research.artifacts.unshift(artifact);
    state.research.selectedArtifact = artifact;
    if (response.data.run_id) {
      try {
        await loadResearchRun(response.data.run_id, { render: false });
      } catch (error) {
        setStatus(error.message);
      }
    }
  ...
  async function queueLocalLibraryUploadIngest() {
    ...
    const response = await fetch(
      endpoints.localLibraryUpload,
      withProjectHeaders(endpoints.localLibraryUpload, {
        method: "POST",
        body: formData,
      }),
    );
  ...
  function replaceResearchArtifact(updatedArtifact) {
    state.research.artifacts = [
      updatedArtifact,
      ...state.research.artifacts.filter((artifact) => artifact.id !== updatedArtifact.id),
    ];
    if (state.research.selectedArtifact && state.research.selectedArtifact.id === updatedArtifact.id) {
      state.research.selectedArtifact = updatedArtifact;
    }
  }

  async function saveResearchArtifact(artifactId, format) {
    const artifact = state.research.artifacts.find((item) => item.id === artifactId);
    if (!artifact) {
      throw new Error("Research artifact is no longer available.");
    }
    const response = await fetchJson(`${endpoints.researchArtifacts}/${artifactId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        is_saved: true,
        saved_format: format || "markdown",
        saved_title: artifact.saved_title || artifact.title,
      }),
    });
    replaceResearchArtifact(response.data);
    state.research.selectedArtifact = response.data;
    renderAll();
    setStatus(`Saved ${response.data.saved_title || response.data.title}.`);
  }

  function downloadResearchArtifact(artifactId, format) {
    const artifact = state.research.artifacts.find((item) => item.id === artifactId);
    if (!artifact) {
      throw new Error("Research artifact is no longer available.");
    }
    if (artifact.status !== "completed") {
      throw new Error("Wait until the research run completes before exporting.");
    }
    const resolvedFormat = format || artifact.saved_format || "markdown";
    const text = artifactDownloadText(artifact, resolvedFormat, {
      formatArtifactType,
      previewText: (item) => renderResearchArtifactPreview(item).replace(/<[^>]+>/g, ""),
    });
    const extension = resolvedFormat === "csv" ? "csv" : (resolvedFormat === "script" ? "json" : "md");
    downloadTextFile({
      text,
      filename: `${(artifact.saved_title || artifact.title || "arxie-output").replace(/[^a-z0-9]+/gi, "-").replace(/^-|-$/g, "").toLowerCase()}.${extension}`,
      mimeType: resolvedFormat === "csv" ? "text/csv" : "text/plain",
    });
    setStatus(`Exported ${artifact.saved_title || artifact.title}.`);
  }
  ...
  if (elements.paperSourceSearchInput) {
    elements.paperSourceSearchInput.addEventListener("input", () => {
      renderPaperSources();
    });
  }
  ...
  elements.projectOpenForm.addEventListener("submit", async (event) => {
    event.preventDefault();
    try {
      setStatus("Opening project…");
      await openProjectFromForm();
